...
extension TrieDictionary {
    /**
     ...
     ```swift
     let numbers: TrieDictionary<Int> = ["one": 1, "two": 2]
     let strings = numbers.mapValues { String($0) }
     // Result: ["one": "1", "two": "2"]
     ```
    ...
    /**
     ...
     ```swift
     let mixed: TrieDictionary<String> = ["one": "1", "two": "abc", "three": "3"]
     let numbers = mixed.compactMapValues { Int($0) }
     // Result: ["one": 1, "three": 3]
     ```
     
     - Parameter transform: A closure that transforms each value, returning nil to exclude entries
